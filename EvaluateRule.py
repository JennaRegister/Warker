...
files = [("FirstSegment.pkl","segment.csv")]
    ...
    with open('/home/Jenna/Desktop/Warker/'+str(f[1]), 'w') as f:
        for damt in xrange(0,1000):
                ...
                for h in space:
                    ...
                    #print "FOR TARGET: Numerator: " + str(float(len(set(h.ll_counts.keys()) & set(target)))) + " Denom of Recall: " + str(len(target)) + " Denom of Precision: " + str(len(set(h.ll_counts.keys()))) + " Likelihood(per datap): " + str(h.likelihood) + " Prior: "+ str(h.prior)
                    #print "FOR TRAINING: Numerator: " + str(float(len(set(h.ll_counts.keys()) & set(training)))) + " Denom of Recall: " + str(len(training)) + " Denom of Precision: " + str(len(set(h.ll_counts.keys()))) + " Likelihood(per datap): " + str(h.likelihood) + " Prior: "+ str(h.prior)
